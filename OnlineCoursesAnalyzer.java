...
public class OnlineCoursesAnalyzer {
  ...
  public Map<String, Integer> getPtcpCountByInst() {
    Map<String, Integer> ptcp = courses.stream()
  ...
  public Map<String, Integer> getPtcpCountByInstAndSubject() {
    Map<String, Integer> ptcp = courses.stream()
        .collect(Collectors.groupingBy(Course::getInstructorsAndSubject,
            Collectors.summingInt(Course::getParticipants)));
  ...
  public List<String> getCourses(int topK, String by) {
    ...
    if (by.equals("hours")) {
      list = courses.stream().sorted(Comparator.comparing(Course::getTotalHours).reversed())
          .distinct().toList();
    } else {
      list = courses.stream().sorted(Comparator.comparing(Course::getParticipants).reversed())
          .distinct().toList();
    }
  ...
  public List<String> searchCourses(String courseSubject, double percentAudited,
    ...
    List<String> tmp = courses.stream()
        .map(course -> course.getTitle().toLowerCase(Locale.ROOT)).toList();
    List<Course> courseList = courses.stream()
            .filter(course ->
            course.getSubject().toLowerCase(Locale.ROOT).contains(courseSubject.toLowerCase(
                Locale.ROOT))
  ...
  public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) {
    ...
    Map<String, String> courseLatest = courses.stream()
        .sorted(Comparator.comparing(Course::getLaunchDate).reversed())
        .collect(Collectors.groupingBy(Course::getNumber,
            Collectors.collectingAndThen(Collectors.toList(),
                courses1 -> courses1.get(0).getTitle())));
    ...
    for (int i = 0; i < courses.size(); i++) {
      ...
      double similarity =
          Math.pow((age - avgMedianAge), 2) + Math.pow((100 * gender - avgMale), 2)
